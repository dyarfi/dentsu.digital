...
$("#delete").on("click",function(){b.remove(b.getActiveObject())});$("#addText").on("click",function(){var a=new fabric.Text($("#canvaText").val(),{left:210,top:110});b.add(a)});$("#send_image").click(function(a){a.preventDefault();var c=$(this).next(".msg");b.discardActiveGroup();b.discardActiveObject();b.renderAll();$.ajax({url:"upload/upload_result",type:"POST",dataType:"json",data:{data:b.toDataURL("image/png"),csrf_token:$.cookie("csrf_cookie")},complete:function(a,b){var d=a.responseJSON;console.log(d);
c.empty();c.html('<div class="alert alert-danger msg"><button class="close" data-close="alert"></button>'+d.result.text+"</div>");1===d.result.code&&setTimeout(function(){window.location.href=base_URL+"fabric"},2E3);"success"==b&&alert("saved!")}})})})();
