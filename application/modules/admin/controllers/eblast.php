...
class Eblast extends Admin_Controller {
    ...
    public function index() {
        ...
        if($rows) {
            ...
            foreach($rows as $row){
                ...
                $temp_rows[$i]->group_id = $this->EblastGroups->getGroupName_ById($row->group_id);
                $i++;
            }
        }
    ...
    public function add() {
	    ...
	    $this->form_validation->set_rules('username', 'Eblastname', 'trim|required|min_length[5]|max_length[24]|xss_clean');
	    ...
	    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		    ...
		    else
		    {
			    ...
			    $this->Eblasts->setEblast($this->input->post());

			    // Set message
			    $this->session->set_flashdata('message','Eblast created!');
	    ...
	    // Eblast Groups Data
	    $data['user_groups']	= $this->EblastGroups->getAllEblastGroup();
    ...
    public function edit($id=0){
	    ...
	    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		    ...
		    if ($this->form_validation->run() == FALSE) {
			    ...
			    // Set data to add to database
			    $this->Eblasts->updateEblast($posts);

			    // Set message
			    $this->session->set_flashdata('message','Eblast updated');
    ...
    public function delete($id){
        ...
        // Delete user data
        $this->Eblasts->deleteEblast($id);

        // Set flash message
        $this->session->set_flashdata('message','Eblast deleted');
    ...
    public function change() {
		if ($this->input->post('check') !='') {
			...
			$this->Eblasts->setStatus($row,$this->input->post('select_action'));
			$this->EblastProfiles->setStatus($row,$this->input->post('select_action'));
		    }
		    // Set message
	...
	public function upload() {
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$profile_id 			= $this->EblastProfiles->setEblastProfiles($profile);
